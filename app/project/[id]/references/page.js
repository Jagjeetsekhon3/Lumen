        ...
        <div style={{ padding: '28px 32px' }}>
          ...
          {refs.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px 40px', background: 'var(--bg2)', border: '1.5px dashed rgba(0,87,255,0.2)', borderRadius: '16px', animation: 'reveal .5s .1s ease both', opacity: 0 }}>
              ...
              <div style={{ fontSize: '13px', color: 'var(--text3)' }}>Upload images and tag them — typography, background, product treatment, color</div>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '14px', animation: 'reveal .5s .1s ease both', opacity: 0 }}>
              {refs.map((ref, i) => <RefCard key={ref.id} ref_={ref} onTagChange={updateTag} onDelete={deleteRef} />)}
              <AddCard onClick={() => uploadRef.current?.click()} />
            </div>
          )}
        </div>
      </main>
  ...
  return (
    <div onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      ...
      <div style={{ aspectRatio: '1', overflow: 'hidden', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,87,255,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: hover ? 1 : 0, transition: 'opacity .25s' }}>
          <button onClick={() => onDelete(ref_.id)} style={{ background: 'rgba(255,255,255,0.2)', border: '1px solid rgba(255,255,255,0.3)', borderRadius: '8px', padding: '8px 14px', color: '#fff', fontSize: '12px', cursor: 'pointer', fontFamily: 'var(--font-ui)' }}>Remove</button>
        </div>
      </div>
...
function AddCard({ onClick }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ borderRadius: '12px', border: `1.5px dashed rgba(0,87,255,${hover ? '.5' : '.2'})`, aspectRatio: '1', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: hover ? 'var(--blue-light)' : 'transparent', transition: 'all .3s', gap: '8px' }}>
      <span style={{ fontSize: '24px', color: 'var(--blue)', opacity: hover ? 1 : .4 }}>+</span>
      <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'var(--text3)' }}>Add reference</span>
    </div>
  )
}
